---
title: "Quarto Assignment-Learning r in Quarto"
...
This is copied from rpubs.com modified for learning purposes.

# Introduction

Numerous guides have been written on the exploration of this widely
known dataset. Iris, introduced by Ronald Fisher in his 1936 paper *The use of multiple measurements in taxonomic problems*,
contains three plant species (setosa, virginica, versicolor) and four
features measured for each sample. These quantify the morphologic
variation of the iris flower in its three species, all measurements
...
Written packages make it easier to work with datasets than regular
baseR functions. They have been optimized to be faster and more
intuitive than baseR functions, therefore reducing the steepness of the R
learning curve. Let’s take a look;

## The dplyr package

Use `install.packages("dplyr")` in your console to install
this package. Note that you must be connected to the internet. If
you’ve opened a new R script file, you will need to use the keys
...
This is the mean sepal.length for the virginica species. Challenge2:
The standard deviation gives how much individual values vary from the
mean. Find the standard deviation of sepal.length using `summarise()` and `sd()`
